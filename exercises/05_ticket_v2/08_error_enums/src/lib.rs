...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        ...
        Err(ticket_new_error) => match ticket_new_error {
            TicketNewError::TitleError {details} => panic!("{}", details),
            TicketNewError::DescriptionError {details} => Ticket::new(title, "Description not provided".into(), status).unwrap()
        }
    }
}
    ...
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleError {
                details: "Title cannot be empty".into()
            });
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleError {
                details: "Title cannot be longer than 50 characters".to_string()
            });
        }
        ...
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionError {
                details: "Description cannot be longer than 500 characters".to_string()
            });
        }
